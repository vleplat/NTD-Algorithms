...
nb_seeds = 0 # 10, 0 for only plotting
name_store = "xp_acc_noacc_02-09-22DUMMY"
variables={    
    "U_lines" : [40],
    "V_lines" : [40],
    "beta" : [1],
    "ranks" : [[4,5,6]], #todo split?
    "accelerate": [False, 1e-2, 1e-1, 3e-1],#[False, 1e-2, 1e-1, 4e-1],
    "iter_inner": [500],
...
# 3. Making plots
fig = px.line(df_conv_median_it, x="it", y="errors", color="accelerate", log_y=True, facet_col="iter_inner", facet_row="extrapolate", error_y="q_errors_p", error_y_minus="q_errors_m", template="plotly_white")
fig2 = px.line(df_conv_median_time, x="timings", y="errors", color="accelerate", log_y=True, facet_col="iter_inner", facet_row="extrapolate", error_y="q_errors_p", error_y_minus="q_errors_m", template="plotly_white")
# smaller linewidth
fig.update_traces(
    selector=dict(),
    line_width=3,
